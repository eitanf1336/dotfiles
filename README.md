...
| `orphan-reaper` | Every minute (`orphan-reaper.timer`, units in `systemd-user/`) kills headless Chromes and chrome-devtools-mcp servers whose launcher is gone. A stray SwiftShader headless Chrome burned 5 to 7 cores for an hour, three times in one day. `orphan-reaper -n` lists without killing. |
| `takeover` | Hands the whole laptop to one job: `takeover start -l "label" -- <command>` freezes every app scope and non-essential service with the systemd freezer (nothing killed, nothing lost), runs the job at full priority in its own unit, shows a small progress window, and thaws everything when the job exits. `takeover pause` (Ctrl+Alt+P) pauses or resumes; `takeover plan` lists what it would freeze; `takeover status`. Never freezes the desktop, audio, VPN, keep-awake, the WhatsApp bridge and bot, or the terminal that started it. |
| `freshen` | A soft restart without rebooting: runs `makefast -y`, then kills only positively-identified junk (idle Claude spares over 20 min old, orphaned headless Chromes, orphaned chrome-devtools-mcp, orphaned timeout/sleep wrappers) and pulls gnome-shell and Xwayland back out of swap when there is room. Never touches Chrome, Spotify, PolyArena or any service, live Claude chats, or anything it cannot classify. `freshen -n` is a dry run. |
| `memtop` | Live memory task manager grouped by PROGRAM (flatpak, GNOME app, or command), showing real RAM (PSS) and swap per group with Kill buttons; `memtop --text` prints the table once, `--watch` refreshes it in the terminal. `Ctrl+Shift+Esc` opens it. |
| `mc` | Opens Minecraft on the Minecraft Arena server (`~/code/MyProjects/MinecraftArena`), starting the arena server and harness daemon first if they are down, and refusing to open a second client. Takes an optional username. Shadows Midnight Commander, which is unused here. |
| `makefast` | **One command for "my laptop is slow".** Runs every check that has ever actually cost this machine speed, fixes what is safe, and reports the rest honestly. In order: the CPU clock (GNOME leaves you on `power-saver` after a low battery and never restores it, so the chip sits at 1200 MHz of 4600 and *nothing* in `top`/`free`/pressure explains it), the desktop's CPU priority drop-ins, abandoned dev servers still burning cores with their listening socket already closed, permanent low-level burn (cheap per second, a whole core per day), units stuck in a restart storm, dead units, and leaked Claude spares. Ends with load, pressure, RAM and how many Claude agents are actually running, because sometimes the honest answer is "you have six agents". `-y` never asks, `-n` is a dry run, `--bench` times a single-core loop before and after. |
...
| `gpu-primary` | Chooses which GPU GNOME renders the desktop on. Docked with the lid panel off, mutter still picks the **Intel** GPU "from builtin panel presence" even though that GPU drives no display, then CPU-copies every frame to the NVIDIA monitor over a path that is not hardware accelerated and periodically throws `gbm_surface_lock_front_buffer failed` — the random screen jitter that `fix-screen` (Super+F5) only papered over. `gpu-primary nvidia` tags the dGPU as mutter's preferred primary so the main monitor is a direct scanout instead. `intel` reverts, `status` shows what the running session actually picked and the failure count this boot. Takes effect at the next login; a boot guard reverts it automatically if the desktop ever stops coming up. See `display-jitter/README.md`. |
